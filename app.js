...
let DOL=[],LEC=[],LSRC='fallback',CSRC='fallback',MAC=null,CAU=[],PL=30,yCh=null,tmr;
...
async function ld(){setSt('load','Actualizando...');document.getElementById('rbtn').classList.add('spin');try{await fa();rCZ();rMac();rARB();rCal();rPry();rCN();const live=[];if(DOL.length>0)live.push('Dólar');if(MAC?.source==='live')live.push('Macro');if(LSRC==='byma')live.push('LECAPs');if(CSRC==='bcra'||CSRC==='mae')live.push('Caución');const missing=[];if(!DOL.length)missing.push('Dólar');if(MAC?.source!=='live')missing.push('Macro');setSt('ok',missing.length?'Parcial — Sin '+missing.join(', '):'En vivo — '+live.join(', '));stm();syncRlTna();}catch(e){setSt('err','Error');}finally{document.getElementById('rbtn').classList.remove('spin');}}
